...
export default function AuthScreen() {
  ...
  return (
    <main className="min-h-screen bg-[#0b0813] text-white grid lg:grid-cols-2">
      <section className="hidden lg:flex p-16 flex-col justify-between bg-gradient-to-br from-[#17102b] to-[#0b0813]" aria-label="Welcome to Questlish">
        <div className="flex items-center gap-3 text-2xl font-bold"><span className="p-2.5 rounded-xl bg-violet-600"><Zap aria-hidden="true" /></span>Questlish</div>
      ...
      <section className="flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-[#151026] border border-violet-800/40 rounded-3xl p-7 sm:p-9 shadow-2xl">
          <div className="lg:hidden flex items-center gap-2 mb-8 font-bold text-xl"><Zap className="text-violet-400" />Questlish</div>
          ...
          <form onSubmit={submit} noValidate>
            {mode === 'register' && <Field ref={firstInput} label="Username" name="name" value={form.name} onChange={change} autoComplete="name" minLength={2} />}
            {mode === 'register'
              ? <Field ref={mode === 'register' ? undefined : firstInput} label="Email address" name="email" type="email" value={form.email} onChange={change} autoComplete="email" invalid={emailInvalid} help={emailInvalid ? 'Enter a valid email, for example name@example.com.' : ''} />
              : <Field ref={firstInput} label="Email or username" name="identifier" value={form.identifier} onChange={change} autoComplete="username" />}
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-3 top-10 p-1.5 text-gray-300 hover:text-white rounded-lg" aria-label={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? <EyeOff size={19} /> : <Eye size={19} />}</button>
            </div>
